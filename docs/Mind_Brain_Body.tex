...
\tightlist
\item
  Create MBB calendar event session 1 and session 2 and invite researcher
\item
  Create MBB calendar event to mail study package to participant
\item
  Create MBB calendar event session 1 reminder 1 (email) (1 week prior)
\item
  Create MBB calendar event session 1 reminder 2 (email and call) (3 days prior)
\item
  Create MBB calendar event session 1 links email (day of first session- right before)
\item
  Create MBB calendar event to send session 2 confirmation email (email) (day of first session- right after)
\item
  Create MBB calendar event session 2 reminder 1 (call) (day before second session)
\item
  Create MBB calendar event to send session 2 reminder 1 (email) (day of second session- right before)
\end{itemize}
...
\tightlist
\item
  Send session 1 reminder 1 email (in templates - attach next steps, consent/assent, zoom instructions, researcher information, ``What is involved in this study?'' video, confirm if package is received)
\item
  Send session 1 reminder 2 email (in templates - attach previous, confirm if package is received)
\item
...
\emph{You will also be answering some surveys with the researcher.}
...
\tightlist
\item
  One is the hair sample which helps measure stress hormones
\item
...
\emph{Once you are all set with that, we'd like you for to grab the box we sent you in the mail. This contains everything you will need for today's session and what you will do at home after today's session.}

\emph{The box should contain a sheet of paper at the top which details all of the contents of the box, which we will unpack now!}
...
\tightlist
\item
  Reward board with gold star stickers 2 pencils
\item
  A session booklet
\item
  Parent surveys booklet
\item
  Child surveys booklet
\item
...
\item
  Cardboard box with bag and cotton balls inside Mind Brain Body information card
\item
  Return mailer with FedEx slip attached on the top
\end{enumerate}

\emph{Please set aside the box for now but keep the materials near you as we may need to access materials from the box throughout today's session!}
...
\emph{You can now enter your MBB number on the screen}

{[}Once confirmed, let the parent know they can start on their surveys and the child can run through the task{]} \emph{Great! That sounds right. So, you can get started on the task while {[}child's name{]} runs through this activity for about 15-20 minutes. It will tell you once the task is over, and we can regroup!}
...
\emph{This next part is about your child's learning and memory so it is important they dont get help from you! We actually prefer you don't watch the screen so you dont know which pictures they see. If you recall the example from our powerpoint at the beginning of this session, this will be the time that your child will see some pictures on the screen}

{[}Researcher to bring back up powerpoint slide with example Halloween stimuli if requested{]}

\emph{While you wait for your child to complete their task, you can get started on some parent surveys. So now I will ask you to reach into your Session Package and pull out the booklet titled ``Parent Survey Booklet'' and a pencil to fill these out.}

{[}Wait for parent to pull the parent survey booklet out{]}

\emph{We ask that you please fill the surveys out in the order that they are presented. This next part with your child will take around 15-20 minutes and I will notify you when we are finished with this next part. Don't worry- you will have time to return to the surveys during another part of this session! Once we get your child get set up with the activity, you can work on the survey booklet.}

Make sure the parent knows that some of the questionnaires are Parent Proxy (about their child(ren)) and others are Parent Self (about themselves).
...
\emph{Next, we will move on to some Child Surveys. I will ask you to now reach into your Session Package and pull out the ``Child Survey Booklet'' and a pencil.}
